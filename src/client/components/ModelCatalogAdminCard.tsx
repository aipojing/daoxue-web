        ...
        <>
          ...
          <form className="catalog-admin-form catalog-model-form" aria-describedby={error ? 'catalog-admin-error' : undefined} onSubmit={(event) => { event.preventDefault(); void saveModel(); }}>
            ...
            <div className="form-row">
              <label htmlFor="catalog-model-endpoint">端点<select id="catalog-model-endpoint" value={modelDraft.endpointId} onChange={(event) => setModelDraft((draft) => ({ ...draft, endpointId: Number(event.target.value) }))} required disabled={pending}><option value={0}>请选择端点</option>{providers.flatMap((provider) => provider.endpoints).map((endpoint) => <option key={endpoint.id} value={endpoint.id}>{endpoint.adapterType} / {endpoint.baseUrl}</option>)}</select></label>
