...
- 共享兜底由管理员显式控制，且是 **fail closed**：只有 `shared_ai_fallback_enabled` 明确等于 `'1'` 时才使用共享服务；记录缺失、被误删或设置读取异常（返回空配置）都按"关闭"处理，不会在管理员不知情时消耗共享 Key。migration 0009 初始写入 `'1'` 以保证发布平滑；完成用户迁移后由管理员在「AI 服务」页关闭，进入严格 BYOK。
